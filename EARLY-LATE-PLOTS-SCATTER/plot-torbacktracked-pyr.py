...
if __name__ == "__main__":
    ...
    for molecule, ntraj, run_path, numatoms  in zip(molecules, Ntraj, runpaths, Natoms):
        ...
        for i in range(1,ntraj):
            ...
            if os.path.exists('DONT_ANALYZE'):
                ...
                #print(orders)
                for21 = collect_geom_param_hops(run_path + 'crossing-s2-s1.xyz',orders,'TRAJ_'+str(i),numatoms,pyrs)
                #print(for21)
                for10 = collect_geom_param_hops(run_path + 'crossing-s1-s0.xyz',orders,'TRAJ_'+str(i),numatoms,pyrs)
                #print(for10)
                back21 = collect_geom_param_hops(run_path + 'crossing-s1-s2.xyz',orders,'TRAJ_'+str(i),numatoms,pyrs)
    ...
    ax[0].scatter(96.1,0.8, marker='D', label=r'S$_1$/S$_0$-Tw-MECI', color='gold', edgecolors='k', zorder=20)
    ax[0].scatter(57.6,46.9, marker='s', label=r'S$_1$/S$_0$-CIpy', color='gold',edgecolors='k', zorder=21)       # NEED to change to CAS(10,8), currently CAS(14,12)
    ax[0].scatter(0,0, marker='*', s=100.0, label=r'S$_2$/S$_1$-HTI', color='gold',edgecolors='k', zorder=22)       
    ax[0].scatter(111.8,1.0, marker='P', s=60.0, label=r'S$_2$/S$_1$-Tw-MECI', color='gold',edgecolors='k', zorder=22)       
    ax[1].scatter(98.9,0.8, marker='D', label=r'S$_1$/S$_0$-Tw-MECI', color='gold', edgecolors='k', zorder=23)
    ax[1].scatter(58.0,43.4, marker='s', label=r'S$_1$/S$_0$-CIpy', color='gold',edgecolors='k',zorder=24)
    ax[1].scatter(0,0, marker='*', s=100.0, label=r'S$_2$/S$_1$-HTI', color='gold',edgecolors='k', zorder=25)       
    ax[1].scatter(79.9,2.3, marker='P', s=60.0, label=r'S$_2$/S$_1$-Tw-MECI', color='gold',edgecolors='k', zorder=22)       
    ax[0].set_xlim(-5,120)
    ax[0].set_ylim(-5,90)
    ax[1].set_xlim(-5,120)
    ax[1].set_ylim(-5,90)
