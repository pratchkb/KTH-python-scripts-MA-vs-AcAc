...
def calculate_param(filename, atom_order):
    ...
    for line in fdata:
        ...
        if "Frame " in line:
             ...
             if h1 > h2: 
                 tor.append(geom_param.compute_torsion(data,0,2,3,4))
             else:
                 tor.append(geom_param.compute_torsion(data,3,2,0,1))
             ...
             Htran.append((geom_param.compute_bond(data,[4,13])-geom_param.compute_bond(data,[1,13])))
...
if __name__ == "__main__":
    ...
    path = "/data/projects/Pratip_MA_vs_AcAc/AcAc-data/2D-plot-data/BAGEL-lint-ecup-symCI1412-ecupreversed/"
    ...
    ax[0].plot(ht,(data[:,2]-num)*27.2114, color='blue', label='S$_1$')
...
#ax[0].legend(loc="upper left", ncol=2, fontsize=10, frameon=False)
    ...
    plt.xlim(-0.6032987484679846,0.6032987484679846)
    plt.subplots_adjust(wspace=0, hspace=0.10)
    #plt.savefig(pathscript + 'Trial_AcAc_Htransfer_linearint_Ecup.png', dpi=1000)
    plt.savefig(pathscript + 'Trial_AcAc_Htransfer_linearint_Ecup.pdf', dpi=1000)
