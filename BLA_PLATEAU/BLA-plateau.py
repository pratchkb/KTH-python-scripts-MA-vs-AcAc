...
if __name__ == "__main__":
    ...
    path_script = '/data/projects/Pratip_MA_vs_AcAc/PLOTS/BLA_PLATEAU/'
    for molecule, ntraj, run_path, numatoms  in zip(molecules, Ntraj, runpaths, Natoms):
            ...
            if os.path.exists('DONT_ANALYZE'):
                ...
                if os.path.exists('BACKHOP'):
                    ...
                    for time,density in zip(densities[0], densities[1]):
                        time_totalpl.append(time)
                        density_totalpl.append(density)
                    ...
                    ht = np.insert(densities[1], pos, np.nan)
                    #print(time,ht)
                    ax2.plot(time, ht, color = clr[1], alpha=0.9, linewidth=0.4, zorder=0)
                    os.chdir(run_path) 
        ...
        plt.savefig(path_script + f'{molecule}-BLA-plateau.png', dpi=500)
        plt.savefig(path_script + f'{molecule}-BLA-plateau.pdf', dpi=500)
...
#
        f=open(path_script + f'{molecule}-datapl.dat','w')
        for i,j in zip(time_totalpl, density_totalpl):
            f.write('{}\t{}\n'.format(i,j))
        f.close()
        ...
        f.close()
#
        datapl = np.genfromtxt(path_script + f'{molecule}-datapl.dat',dtype='float64')
...
#
        H, xedges, yedges = np.histogram2d(time_totalpl, density_totalpl, bins=100, density=True, range=[[0.0,200.0],[-0.5,0.5]])
        H2, xedges, yedges = np.histogram2d(time_total, density_total, bins=100, density=True, range=[[0.0,200.0],[-0.5,0.5]])
        ...
        N_gau_conv2 = ndimage.convolve(H2, gau2d, mode='constant', cval=0.0)
...
#
        ...
        cbar = ax1.contourf(X, Y, N_gau_conv, norm=colors.Normalize(vmin=N_gau_conv.min(), vmax=N_gau_conv.max()), cmap='gist_heat_r', levels=np.linspace(N_gau_conv.min(),N_gau_conv.max(),51), zorder=10)
        cbar = ax2.contourf(X, Y, N_gau_conv2, norm=colors.Normalize(vmin=N_gau_conv2.min(), vmax=N_gau_conv2.max()), cmap='gist_heat_r', levels=np.linspace(N_gau_conv2.min(),N_gau_conv2.max(),51), zorder=10)
...
#        
        ...
        plt.tight_layout()
        plt.subplots_adjust(wspace=0.05)
        fig.savefig(path_script + f'{molecule}-BLA-plateau-density-updated.png', dpi=500)
        fig.savefig(path_script + f'{molecule}-BLA-plateau-density-updated.pdf', dpi=500)
#
