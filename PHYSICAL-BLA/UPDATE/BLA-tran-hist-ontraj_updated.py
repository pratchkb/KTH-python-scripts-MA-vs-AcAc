...
if __name__ == "__main__":
    ...
    path_script = '/data/projects/Pratip_MA_vs_AcAc/PLOTS/PHYSICAL-BLA/UPDATE/'
    for molecule, ntraj, run_path, numatoms  in zip(molecules, Ntraj, runpaths, Natoms):
        ...
        xedges = np.linspace(0.0,200,400)
        yedges = np.linspace(-0.5,0.5,400)
        X, Y = np.meshgrid(xedges, yedges)
        sigma_x = 0.1
        sigma_y = 0.1
        gau2d = np.exp(-0.5 * (X-100)**2/sigma_x**2 - 0.5 * (Y)**2/sigma_y**2 )

        H, xedges, yedges = np.histogram2d(time_total, density_total, bins=400, density=True, range=[[0.0,200.0],[-0.5,0.5]])
...
#        sigma_y = 0.001
        ...
        #cbar = ax.contourf(X,Y,H,cmap='gist_heat_r',levels=np.linspace(H.min(),H.max(),21))
        #fig.colorbar(cbar, ax=ax)
        cbar = ax.contourf(X, Y, N_gau_conv, norm=colors.Normalize(vmin=N_gau_conv.min(), vmax=N_gau_conv.max()), cmap='gist_heat_r', levels=np.linspace(N_gau_conv.min(),N_gau_conv.max()), zorder=10) #levels=np.linspace(N_gau_conv.min(),N_gau_conv.max(),51))
        fig.colorbar(cbar, ax=ax)
        ax.set_xlim(0,200)
        ax.set_ylim(-0.5,0.5)
        ax.set_xlabel(r"Time (fs)", fontsize=18)
        ...
        fig = plt.figure(figsize=(6,3))
        gs = GridSpec(1,2, width_ratios=[3, 1])
        clr = ['#0072BD', 'orange', '#66BF2E', 'k', 'green', 'darkgreen', 'darkblue', 'orangered', '#D45500', 'crimson', 'dodgerblue', 'lime']
        ax1 = fig.add_subplot(gs[0])
        ax2 = fig.add_subplot(gs[1])

        cbar = ax1.contourf(X, Y, N_gau_conv, norm=colors.Normalize(vmin=N_gau_conv.min(), vmax=N_gau_conv.max()), cmap='gist_heat_r', levels=np.linspace(N_gau_conv.min(),N_gau_conv.max()), zorder=10)
