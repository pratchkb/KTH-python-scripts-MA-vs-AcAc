...
def collect_geom_param(filename, atom_order, natoms):
    ...
    for line in fdata:
        ...
        if "t=  " in line:
             ...
             bla.append(calculate_bla4(data,atom_order)[0])

    fdata.close() 

    os.system('rm -r tmp')

    return timstp, Htran, bla 
    ...
    for line in fdata:
        ...
        if "t=  " in line:
            ...
            if int(st) == 3:         # S2 state
                ...
                blas2.append(calculate_bla4(data,atom_order)[0])
            ...
            if int(st) == 2 or int(st) == 1:         # S1 or S0 state
                ...
                blas1.append(calculate_bla4(data,atom_order)[0])

    os.system('rm -r tmp3')
    #os.chdir(path)

    return timstps2, Htrans2, blas2, timstps1, Htrans1, blas1
    ...
    for molecule, ntraj, run_path, numatoms  in zip(molecules, Ntraj, runpaths, Natoms):
        ...
        for i in range(1,ntraj):
            ...
            if os.path.exists('DONT_ANALYZE'):
                ...
                ht = np.insert(densities[2], pos, np.nan)
                #print(time,ht)
                ax1.plot(time, ht, color = clr[1], alpha=0.9, linewidth=0.4, zorder=0)
  
                for21 = collect_geom_param_hops(run_path + 'crossing-s2-s1.xyz',atom_order,'TRAJ_'+str(i),numatoms)
                #print(for21)
                for10 = collect_geom_param_hops(run_path + 'crossing-s1-s0.xyz',atom_order,'TRAJ_'+str(i),numatoms)
                #print(for10)
                back21 = collect_geom_param_hops(run_path + 'crossing-s1-s2.xyz',atom_order,'TRAJ_'+str(i),numatoms)

                tim21 = []
                bla21 = []
                for elem,blaelem in zip(for21[0],for21[1]):
                    tim21.append(elem)
                    bla21.append(blaelem)


                tim12 = []
                bla12 = []
                for elem,blaelem in zip(back21[0],back21[1]):
                    tim12.append(elem)
                    bla12.append(blaelem)

                if molecule == "MA":
                    if for21[0] != 0:
                        ax1.scatter(tim21, bla21, s=0.6, c=clr[8], zorder=5)
                    if back21[0] != 0:
                ...
                htht1 = np.insert(densit[2], posht1, np.nan)
                for elem in htht1:
                    HT1.append(elem) 
                ...
                posht2 = np.where(np.abs(np.diff(densit[3])) > 0.5)[0] + 1
                timeht2 = np.insert(densit[3], posht2, np.nan)
                htht2 = np.insert(densit[5], posht2, np.nan)
        ...
        ax2.hist(HT1, bins=40, range=[-1,1], density=True, weights=None, orientation='horizontal', color = 'orange', alpha = 0.9, zorder=10)
        ax2.hist(HT2, bins=40, range=[-1,1], density=True, histtype='step', orientation='horizontal', weights=None, color = 'blue', alpha = 0.9, zorder=10)
        ...
        ax2.set_xticks([0.0,2.0,4.0])
        ax1.minorticks_on()
        ax2.minorticks_on()
        plt.tight_layout()
        plt.subplots_adjust(wspace=0.25)
        plt.savefig(path_script + f'{molecule}-BLA-density-colored.png', dpi=500)
        plt.savefig(path_script + f'{molecule}-BLA-density-colored.pdf', dpi=500)
