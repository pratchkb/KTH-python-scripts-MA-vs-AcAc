...
if __name__ == "__main__":
    ...
    path ="/data/projects/Pratip_MA_vs_AcAc/AcAc-data/ACETYLACETONE-ground-state-tests/staggered/B3LYP-D3/HESSIAN-bin/Wigner_298.15K_5000/wigner_298.15K_IC5000/"
    ...
    plt.figure()
    plt.hist(KE_list, bins=30, range=[0,4.5], density=True, weights=None, color = 'orange', alpha = 0.9, label='All')
    plt.hist(KE_list_12, bins=30, range=[0,4.5], density=True, weights=None, color = 'b', histtype='step', stacked=True, linewidth = 3.0, label='Avg. r(CH) \n < 1.2 $\AA$', fill=False)
    plt.axvline(x=1.675, color = 'k', linestyle='--', label='0.5*ZPE')
    ...
    plt.tight_layout()
    plt.savefig('test-KE-wigner-298.15K-AcAc.pdf',dpi=300)
