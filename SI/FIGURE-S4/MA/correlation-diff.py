...
def read_bagel_xmscaspt2_results(nstates):
    ...
    for isample in range(1,1891,1):          # 1785 very specific right now needs to change
   
        #x = str(isample).zfill(4) 
    # Grab excited state energies and oscillator strengths
        results_file = '/data/projects/Pratip_MA_vs_AcAc/MA-data/ABSORPTION/CALC-SSSR/' + str(isample) + '/input' + str(isample) + '.out'  #os.getcwd() + '/x{:04d}/{}'.format(isample,outname)
        xyz = '/data/projects/Pratip_MA_vs_AcAc/MA-data/ABSORPTION/CALC-SSSR/' + str(isample) + '/' + str(isample) + '.xyz'
        #print(xyz)
        ...
        with open(results_file, 'r') as f_in:
            for line in f_in:
                ...
                if ' * Transition    2 - 0 :' in line:
                    ...
                    #compute sum of angles
                    angles.append(geom_param.compute_angle(dat,0,2,3)+geom_param.compute_angle(dat,2,3,4)+geom_param.compute_angle(dat,3,4,1))
                     
                   
                    #compute pyramidalization
                    pyr.append(geom_param.compute_pyramidalization(dat,3,2,4,6))

                    #O-H distances
                    Hbond1.append(geom_param.compute_bond(dat,[0,8]))
                    Hbond2.append(geom_param.compute_bond(dat,[1,8]))

                    #torsion
                    torsion1.append(geom_param.compute_torsion(dat,0,2,3,4))
                    torsion2.append(geom_param.compute_torsion(dat,2,3,4,1))
        ...
        for energy, ang, pr, h1, h2, t1, t2, bla, blaco, blacc in zip(ener_diff, angles, pyr, Hbond1, Hbond2, torsion1, torsion2, BLA, BLACO, BLACC):      #### main energy_diff
...
plt.figure()
plt.hist2d(endiff2, oscdiff, bins=100, cmap='gist_heat_r')
plt.xlabel(r'S$_2$ energy (eV)', fontsize=16)
plt.ylabel('Osc diff', fontsize=16)
plt.ylim(0.0,0.4)
plt.savefig('s2-oscdiff-2.png', dpi=300)
...
plt.xlim(0.0,4.5)
plt.minorticks_on()
plt.savefig('diff-angs-2.png', dpi=300)
...
plt.ylim(-40,40)
plt.savefig('diff-pyr-2.png', dpi=300)
...
plt.ylim(0.5,2.0)
plt.savefig('diff-h1-2.png', dpi=300)
...
plt.ylim(0.5,2.0)
plt.savefig('diff-h2-2.png', dpi=300)
...
plt.ylim(-40,40)
plt.savefig('diff-tor1-2.png', dpi=300)
...
plt.ylim(-40,40)
plt.savefig('diff-tor2-2.png', dpi=300)
...
plt.xlim(3.5,6.5)
plt.minorticks_on()
plt.savefig('diff-ht-2.png', dpi=300)
...
plt.xlim(3.5,6.5)
plt.minorticks_on()
plt.savefig('diff-BLA-2.png', dpi=300)
...
plt.xlim(3.5,6.5)
plt.minorticks_on()
plt.savefig('diff-BLACO-2.png', dpi=300)
...
plt.xlim(3.5,6.5)
plt.minorticks_on()
plt.savefig('diff-BLACC-2.png', dpi=300)
...
xedges = np.linspace(0.0,4.4,80)
yedges = np.linspace(350,380,80)
X, Y = np.meshgrid(xedges, yedges)
sigma_x = 0.10
sigma_y = 1.
gau2d = np.exp(-0.5 * (X-2.2)**2/sigma_x**2 - 0.5 * (Y-365)**2/sigma_y**2 )
H, xedges, yedges = np.histogram2d(endiff2, angs, bins=80, density=True, range=[[0.0,4.4],[350,380]])
#
#
...
plt.tight_layout()
plt.savefig('cont-S2en-contourf-gau-ang-diff.pdf', dpi=300)
...
sigma_x = 0.10
sigma_y = 0.067
gau2d = np.exp(-0.5 * (X-2.2)**2/sigma_x**2 - 0.5 * (Y-0.0)**2/sigma_y**2 )
H, xedges, yedges = np.histogram2d(endiff2, bl, bins=80, density=True, range=[[0.0,4.4],[-1.0,1.0]])
#
#
#    # Histogram2D in matplotlib does not follow Cartesian convention (see Notes),
#    # Please note that the histogram does not follow the Cartesian convention where x values are on the abscissa and y values on the ordinate axis. 
...
plt.yticks(fontsize=18)
plt.tight_layout()
plt.savefig('cont-S2en-contourf-gau-BLA-diff.pdf', dpi=300)
...
sigma_y = 0.10
gau2d = np.exp(-0.5 * (X-2.2)**2/sigma_x**2 - 0.5 * (Y-0.0)**2/sigma_y**2 )
H, xedges, yedges = np.histogram2d(endiff2, ht, bins=80, density=True, range=[[0.0,4.4],[-1.5,1.5]])
#
#
#    # Histogram2D in matplotlib does not follow Cartesian convention (see Notes),
#    # Please note that the histogram does not follow the Cartesian convention where x values are on the abscissa and y values on the ordinate axis. 
...
plt.yticks(fontsize=18)
plt.tight_layout()
plt.savefig('cont-S2en-contourf-gau-ht-diff.pdf', dpi=300)
...
plt.yticks(fontsize=18)
plt.tight_layout()
plt.savefig('cont-S2en-contourf-gau-osc-diff.pdf', dpi=300)
...
sigma_y = 0.01
gau2d = np.exp(-0.5 * (X-2.2)**2/sigma_x**2 - 0.5 * (Y-0.2)**2/sigma_y**2 )
H, xedges, yedges = np.histogram2d(ends2, oscdiff, bins=80, density=True, range=[[0.0,0.4],[0.0,0.4]])
#
#
#    # Histogram2D in matplotlib does not follow Cartesian convention (see Notes),
#    # Please note that the histogram does not follow the Cartesian convention where x values are on the abscissa and y values on the ordinate axis. 
...
plt.yticks(fontsize=18)
plt.tight_layout()
plt.savefig('cont-S2-contourf-gau-osc-diff.pdf', dpi=300)
