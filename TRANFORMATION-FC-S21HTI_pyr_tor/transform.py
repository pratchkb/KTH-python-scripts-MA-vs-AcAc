...
if __name__ == "__main__":
    ...
    path_script = '/data/projects/Pratip_MA_vs_AcAc/PLOTS/TRANFORMATION-FC-S21HTI_pyr_tor/'
    ...
    for molecule, ntraj, run_path, numatoms  in zip(molecules, Ntraj, runpaths, Natoms):
        ...
        if molecule == "MA":
            ...
            pyrs = [3,2,4,6]
            os.chdir(run_path)
            FC = calculate_pyr_atoms(run_path + 'total-FC.xyz', molecule, numatoms, pyrs)
            #for10 = collect_geom_param(runpath + 'crossing-s1-s0.xyz', numatoms)
            S21 = calculate_pyr_atoms(run_path + 'crossing-s2-s1.xyz', molecule, numatoms, pyrs)

            FCt = calculate_main_torsion(run_path + 'total-FC.xyz', molecule, numatoms, hh1, hh2)
            S21t = calculate_main_torsion(run_path + 'crossing-s2-s1.xyz', molecule, numatoms, hh1, hh2)


            ax[0,0].hist(FC, bins=40, histtype='bar', color='orange', alpha=0.9, density=True, range=[-90.0,90.0], label='ICs')
            ax[0,0].hist(S21, bins=40, histtype='step', color='blue', alpha=0.9, linewidth=1.5, density=True, range=[-90.0,90.0], label=r'S$_2$$\rightarrow$S$_1$ hops')
            ax[0,1].hist(FCt, bins=40, histtype='bar', color='orange', alpha=0.9, density=True, range=[-90,90.0], label='ICs')
            ax[0,1].hist(S21t, bins=40, histtype='step', color='blue', alpha=0.9, linewidth=1.5, density=True, range=[-90.0,90.0], label=r'S$_2$$\rightarrow$S$_1$ hops')
        ...
        if molecule == "AcAc":
            ...
            ax[1,0].hist(FC, bins=40, histtype='bar', color='orange', alpha=0.9, density=True, range=[-90.0,90.0], label='ICs')
            ax[1,0].hist(S21, bins=40, histtype='step', color='blue', alpha=0.9, linewidth=1.5, density=True, range=[-90.0,90.0], label=r'S$_2$$\rightarrow$S$_1$ hops')
            ax[1,1].hist(FCt, bins=40, histtype='bar', color='orange', alpha=0.9, density=True, range=[-90,90.0], label='ICs')
            ax[1,1].hist(S21t, bins=40, histtype='step', color='blue', alpha=0.9, linewidth=1.5, density=True, range=[-90.0,90.0], label=r'S$_2$$\rightarrow$S$_1$ hops')
        ...
        ax[0,0].set_xlim(-80.0,80.0)
        ax[1,0].set_xlim(-80.0,80.0)
        ax[0,1].set_xlim(-80.0,80.0)
        ax[1,1].set_xlim(-80.0,80.0)
        ...
        ax[0,1].set_xlabel(r'Torsion ($^{\circ}$)', fontsize=14)
        ax[0,0].set_xlabel(r'Torsion ($^{\circ}$)', fontsize=14)
        ax[1,1].set_xlabel(r'PyrC ($^{\circ}$)', fontsize=14)
        ...
        ax[1,1].legend(fontsize=14, frameon=False, loc='upper right')
        fig.tight_layout()
        fig.savefig(path_script + 'crossing-pyr-tor-updated.pdf',dpi=500)
