...
if __name__ == "__main__":
    ...
    path_script = '/data/projects/Pratip_MA_vs_AcAc/PLOTS/PHYSICAL-HT/UPDATE/'
    for molecule, ntraj, nused, run_path, numatoms  in zip(molecules, Ntraj, Nused, runpaths, Natoms):
        ...
        for i in range(1,ntraj):
            ...
            if os.path.exists('DONT_ANALYZE'):
                ...
                ### new loop for HT per step of all traj
                #for step in range(0,401):
                a.append(densities[1][0])  
        ...
        xedges = np.linspace(0,200,400)
        yedges = np.linspace(-3.0,3.0,400)
        X, Y = np.meshgrid(xedges, yedges)

        H, xedges, yedges = np.histogram2d(data[:,0], data[:,1], bins=400, density=True, range=[[0,200],[-3.0,3.0]])
...
#        fig.colorbar(cbar, ax=ax)
        #cbar = ax.contourf(X, Y, N_gau_conv, cmap='gist_heat_r')  # levels=np.linspace(N_gau_conv.min(),N_gau_conv.max(),51), zorder=10)
        #N_gau_conv, norm=colors.Normalize(vmin=N_gau_conv.min(),vmax=N_gau_conv.max())
        ax.set_xlim(0,200)
        ax.set_ylim(-3.0,3.0)
...
# 
        fig, ax = plt.subplots(figsize=(7,5))
        cbar = ax.contourf(X, Y, N_gau_conv, cmap='gist_heat_r', norm=colors.Normalize(vmin=N_gau_conv.min(),vmax=N_gau_conv.max()), levels=np.linspace(N_gau_conv.min(),N_gau_conv.max(),51), zorder=10) #levels=np.linspace(N_gau_conv.min(),N_gau_conv.max(),51), zorder=10)
        fig.colorbar(cbar, ax=ax)
        plt.xlim(0,200)
        plt.ylim(-3.0,3.0)
        ...
        cbar = ax1.contourf(X, Y, N_gau_conv, norm=colors.Normalize(vmin=N_gau_conv.min(), vmax=N_gau_conv.max()), cmap='gist_heat_r', levels=np.linspace(N_gau_conv.min(),N_gau_conv.max(),51), zorder=1)
        ...
        ax1.scatter(t12, s12, s=0.6, c='lime', zorder=61)
